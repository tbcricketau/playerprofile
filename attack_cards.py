...
def _control_balls(conn, cur, pid, match_ids, hand_like):
    """Same matches, opposition pace to other same-hand top-7 batters."""
    midlist = ",".join(f"'{m}'" for m in match_ids)
    return run_query(f"""
        SELECT D.match_id, D.pitch_line_group_pace_id lin, D.pitch_length_group_pace_1_id len,
               T.team_name opp
        FROM [{DATA_SCHEMA}].[Deliveries] D
        JOIN [{DATA_SCHEMA}].[Teams] T ON D.team_bowling_id = T.team_id
        LEFT JOIN [{DATA_SCHEMA}].[Lookups] LH ON LH.lookup_type_id = 10 AND LH.id = D.striker_hand_id
        WHERE D.match_id IN ({midlist}) AND D.striker_id <> '{pid}' AND D.legal_ball = '1'
          AND D.bowler_pace_spin_id = '1' AND LH.description LIKE '{hand_like}'
          AND TRY_CONVERT(int, D.striker_batting_position) <= 7
    """, conn, cur)
...
def _cells(w_balls, c_balls, LEN, LIN):
    """The FULL standard plan table — every row, every series (Tom 2026-07-15): lengths, lines,
    two composite balls. Each row carries pct/ctrl_pct and a flag: 'more'/'less' (cleared the
    z-gate), 'even' (no real difference), 'thin' (too few balls in this cell to compare).
    Returns [] only when the series itself is too thin to compare at all."""
    def defs():
        yield ("very full", lambda r: LEN.get(r["len"]) in ("<1 m", "1-2 m"))
        for band in ("2-5 m", "5-6 m", "6-8 m", "8-9 m", "9-10 m", "10+ m"):
            yield (LEN_NAME.get(band, band), lambda r, b=band: LEN.get(r["len"]) == b)
        for grp in ("Channel", "Wide Outside Off", "In Line", "Outside Leg"):
            yield (LINE_NAME.get(grp, grp), lambda r, g=grp: LIN.get(r["lin"]) == g)
        yield ("the cut ball (short + wide of off)",
               lambda r: LEN.get(r["len"]) in ("9-10 m", "10+ m")
               and LIN.get(r["lin"]) in ("Channel", "Wide Outside Off"))
        yield ("full at the stumps",
               lambda r: LEN.get(r["len"]) in ("2-5 m", "5-6 m", "6-8 m") and LIN.get(r["lin"]) == "In Line")
    out = []
    nw, nc = len(w_balls), len(c_balls)
    if nw < 60 or nc < 120:                      # too thin to compare diets at all
        return out
    for label, pred in defs():
        cw = sum(1 for r in w_balls if pred(r))
        cc = sum(1 for r in c_balls if pred(r))
        p1, p2 = cw / nw, cc / nc
        z = _z(p1, nw, p2, nc)
        expected = cc * nw / nc
        if cw < 3 and expected < 3:
            ...
            flag = "even"
        out.append({"label": label, "pct": round(100 * p1, 1), "ctrl_pct": round(100 * p2, 1),
...
def _summary(cells, outs_detail):
    """Two readable sentences: the diet, then the dismissals. The table carries the numbers."""
    ranked = sorted([c for c in cells if c["flag"] in ("more", "less")], key=lambda c: -abs(c["z"]))
    pos = [_PROSE.get(c["label"], c["label"]) for c in ranked if c["flag"] == "more"][:3]
    neg = [_PROSE.get(c["label"], c["label"]) for c in ranked if c["flag"] == "less"][:3]
    if pos and neg:
        s1 = f"They bowled you more {_join(pos)}, and less {_join(neg)}."
    elif pos:
        s1 = f"They bowled you more {_join(pos)} than your teammates."
    elif neg:
        s1 = f"They bowled you less {_join(neg)} than your teammates."
    else:
        s1 = "They bowled to you as they bowled to your teammates."
    if not outs_detail:
...
def build_card(conn, cur, pid, name, LEN, LIN, STK):
    ...
    for s in _derive_series(balls)[:N_SERIES]:
        ...
        outs_detail = [{
            ...
            "clip_stem": clip_stem(r["season"], r["gender_id"], r["match_length_id"],
                                   r["match_id"], r["video_file_name"])} for r in outs]
        cells = _cells(pace, ctrl, LEN, LIN)
        cards.append({
            "opp": s["opp"].replace(" M", ""), "d0": s["d0"], "d1": s["d1"],
            "tests": len(s["match_ids"]), "balls": len(sb), "pace_balls": len(pace),
            "runs": runs, "outs": len(outs),
            "avg": round(runs / len(outs), 1) if outs else None,
            "spin_share": round(100 * sum(1 for r in sb if r["ps"] == "2") / len(sb)) if sb else 0,
            "ctrl_balls": len(ctrl), "cells": cells, "dismissals": outs_detail,
            "summary": _summary(cells, outs_detail),
        })
...
def main():
    ...
    for pid in ids:
        ...
        card = build_card(conn, cur, pid, name, LEN, LIN, STK)
        if card:
            out[pid] = card
            n = len(card["series"])
            print(f"  {name:<22} {n} series: " + " | ".join(
                f"v {c['opp']} ({c['tests']}T, {c['balls']}b, {len(c['cells'])} cells)" for c in card["series"]))
        else:
