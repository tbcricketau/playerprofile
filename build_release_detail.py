...
_MIN_BALLS = 150        # below this a cell's average/SR is too unstable to print
_ABS = "ABS(TRY_CAST(D.release_line_unmirrored AS FLOAT))"


def _cells(conn, cur, bowler_id):
    S = DATA_SCHEMA
    ...
    rows = run_query(f"""
        ...
        WHERE {intl} AND D.legal_ball = '1' AND D.bowler_id = '{bowler_id}'
          AND D.release_line_unmirrored IS NOT NULL AND {_ABS} <= 1500
    ...
    for r in rows:
        ...
        out.append({
            ...
            # avg/SR only where the cell is big enough AND actually took wickets
            "avg": round(runs / w, 1) if w >= 3 and b >= _MIN_BALLS else None,
            "sr": round(b / w) if w >= 3 and b >= _MIN_BALLS else None,
            "econ": round(runs / (b / 6), 2) if b >= _MIN_BALLS else None,
            "false_pct": round(100 * int(r["fls"] or 0) / sq, 1) if sq >= 100 else None,
        })
...
def main():
    ...
    for pid, rec in want.items():
        cells, span = _cells(conn, cur, pid)
        out[pid] = {"name": rec.get("name", pid), "seasons": span, "cells": cells,
                    "min_balls": _MIN_BALLS}
        shown = sum(1 for c in cells if c["avg"] is not None)
        print(f"  {rec.get('name', pid)}: {len(cells)} cells ({shown} with a rateable sample), {span}")
        for c in cells:
            print(f"     {c['angle']:5} {c['hand']:3} {c['band']:6} balls={c['balls']:5} "
                  f"wkts={c['wkts']:3} avg={c['avg']} SR={c['sr']}")
    conn.close()
